...
export default class JuegosController {
  public async monitoresDisponibles ({ }) {
  ...
  public async barquito ({ request, response, auth }) {
    ...
    const monitoresFaltantes = await this.monitoresDisponibles({})
    if (monitoresFaltantes.length < 2) {
      return response.badRequest({ 'status': 400, 'mensaje': 'No hay suficientes monitores', 'error': [], 'data': [] })
    }
  }
